...
- [x] **Service worker for offline shell** — Register a service worker that caches the app shell (HTML, CSS, JS, fonts, icons) so the client loads even if the server is temporarily unreachable. Not full offline messaging, but prevents blank page on network hiccup.
